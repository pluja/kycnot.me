...
`APPROVED` means the service passed a **limited, recent review**. It does **not** mean the service is trusted or future-proof.

To be upgraded to `APPROVED`, the service should generally meet these conditions:

- The service is eligible for listing and publicly understandable.
...
> Passed limited checks on specific dates. This does not guarantee future safety.

##### Final Review (Approved -> Verified)

`VERIFIED` means the service showed **consistent behavior over time**. It does **not** mean the service cannot exit scam in the future.
...
- We had at least **2 successful support interactions** on separated dates, when support exists.
- There is **no unresolved scam or blocked-funds pattern**.
- There are **no serious contradictions** in public reports.
...
> Showed consistent behavior over time and passed repeated checks. This is still not a guarantee.
