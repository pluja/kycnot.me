...
Creating an account is very easy and costs almost no effort. We have seen people create accounts only to push a service, attack a competitor, or win an argument. Because of that, the trust weighting does most of the work: a fresh account can comment right away, but its rating barely moves the score until the account has some real history.

When the system detects a coordinated wave of similar comments from many fresh accounts on the same service, those comments are marked as suspicious automatically. The text usually stays visible, but the star ratings are not counted. A moderator reviews each suspicious comment afterward.
...
**Every comment is checked.** An AI reads each new comment with the surrounding context: the service, recent activity, and signals like account age and comment similarity to others posted nearby. Most comments are useful, so the default is to publish quickly: if the AI sees no problems, the comment goes live. Obvious spam is rejected outright. Anything ambiguous, anything making serious claims (KYC issues, blocked funds, scam allegations), and anything that the AI detects as part of a coordinated wave is held for a human moderator. The AI never has the last word on a comment that includes private proof.

We only remove comments that do not follow the guidelines. KYCnot.me is not a forum; comments exist to help users understand services.
