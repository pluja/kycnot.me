...
def _post_crawl(urls: list[str]) -> list[dict[str, Any]]:
    """POST one batch of URLs to crawl4ai and return its result entries.

    Every request goes through here so the payload stays in one place. Crawl4AI
    0.9 rejects a body carrying keys such as deep_crawl_strategy or magic, so a
    single definition is what keeps an upgrade to one edit.
    """
...
def _fetch_crawl4ai(url: str) -> str:
    ...
    results = _post_crawl([url])

    if not results or not results[0].get("success"):
        error = (
            results[0].get("error_message", "unknown") if results else "empty results"
        )
...
# Paths worth following that no document kind claims, so About / Pricing / Blog
# stay out of the corpus. Kinds are matched first, in classify_legal_document_kind.
